    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main',
                    url: 'https://github.com/ADinng/JavaDemo.git',
        ...
        stage('Deploy') {
            steps {
                ...
                script {
                    ...
                    if (env.REMOTE_HOST != null && env.REMOTE_HOST != '') {
                        echo "🚀 Deploying to remote host ${env.REMOTE_HOST}..."
                        def deployPath = '/opt/deployments'
                        def jarFile = "${APP_NAME}-${APP_VERSION}.jar"
                    ...
                    """
                    } else if (sh(script: 'command -v docker', returnStatus: true) == 0) {
                        // Docker 可用 → 本地 Docker 部署
                        echo "🚀 Deploying locally using Docker (port 8081)..."
                        sh """
                            ...
                            docker run -d --name demo-app \
                                ...
                                -p 8081:8081 \
                                openjdk:17 \
                                java -jar /app/${APP_NAME}-${APP_VERSION}.jar
                            echo "Application started locally in Docker (http://localhost:8081)"
                        """
                    } else {
                        // Docker 不可用 → 直接占位
                        echo "🚀 Skipping deployment: no remote host and Docker not available"
                    }
                }
            }
            post {
